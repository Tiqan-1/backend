import { BadRequestException, forwardRef, Inject, Injectable, InternalServerErrorException, Logger, NotFoundException, ForbiddenException } from '@nestjs/common'
...
import { FormElement } from './schemas/assignment-form.schema'
...
export class AssignmentsService {
    ...
    private async checkOwnership(assignmentId: string, managerId: string, updateDto: UpdateAssignmentDto|undefined): Promise<AssignmentDocument> {
        ...
        const createdById = assignment.createdBy._id?.toString() || assignment.createdBy.toString();
    ...
    async publishGrades(assignmentId: string, managerId: string): Promise<void> {
        ...
        if (responses.length === 0) {
            this.logger.warn(`Manager ${managerId} attempt to publish grades for assignment ${assignmentId} which has no submissions.`);
        ...
        if (!allGraded) {
            throw new BadRequestException('Cannot publish grades. One or more student submissions have not been marked as "Graded".');
        }
        ...
        await this.assignmentsRepository.update(
            { _id: assignmentId },
            { gradingState: AssignmentGradingState.PUBLISHED },
        );
    ...
    async findAvailableForStudent(studentId: string): Promise<PaginatedAssignmentStudentDto> {
        ...
        const accessibleLevelIds = await this._getAccessibleLevelIds(studentId);
        ...
        // console.log(" ☺ ☺ ☺ found ☺ ☺ ☺")
        // console.log(found)
        return PaginationHelper.wrapResponse(found.map(assignment => this._mapToStudentAssignmentDto(assignment)), 1, 100, total)
    }


    private async _getAccessibleLevelIds(studentId: string): Promise<string[]> {
        ...
        return activeSubs.map(sub => sub.program).flatMap((program:any) => program.levels);
    ...
    private _mapToStudentAssignmentDto(assignment: AssignmentDocument): StudentAssignmentDto {
        ...
        return {
            ...
            levelName: (assignment.levelId as any)?.name || 'N/A',
